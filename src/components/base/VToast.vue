      ...
      >
        <template v-if="item.timeout > 0 && item.showProgress">
          <div class="v-toast__content__icon" :class="item.type">
            <div class="v-icon" :class="getIcon(item.type)"></div>
          </div>
          <div class="v-toast__content__text">
            <div class="v-toast__content__message">
              <span :style="{ color: colorMessage[item.type] }"
                >{{ noticeMessage[item.type] }}!</span
              >
              {{ item.message }}
            </div>
            <div
              class="v-toast__content__action"
  ...
  methods: {
    ...
    resumeProgress(messageData) {
      ...
      if (messageData.timeout > 0) {
        ...
        this.startProgress(
          messageData,
          messageData.progress * messageData.timeout
        );
      }
    },
    /**
    ...
    removeMessage(messageData) {
      this.toasts = this.toasts.filter(
        (toast) => toast.EmployeeId !== messageData.EmployeeId
      );
    },
    /**
...
.v {
  &-toast {
    ...
    &-icon {
      &-success {
        background: url("@/pages/img/Sprites.64af8f61.svg") no-repeat -1602px -258px;
        width: 20px;
        height: 20px;
      }

      &-error {
        background: url("@/pages/img/Sprites.64af8f61.svg") no-repeat -538px -146px;
        width: 20px;
        height: 20px;
      }

      &-warning {
        background: url("@/pages/img/Sprites.64af8f61.svg") no-repeat -593px -145px;
        width: 22px;
        height: 23px;
      }

      &-info {
        background: url("@/pages/img/Sprites.64af8f61.svg") no-repeat -873px -145px;
        width: 22px;
        height: 22px;
      }

      &-x {
        background: url("@/pages/img/Sprites.64af8f61.svg") no-repeat -1178px -362px;
        width: 12px;
        height: 12px;
      }
    }
